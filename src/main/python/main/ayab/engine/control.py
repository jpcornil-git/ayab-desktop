...
class Control(SignalSender):
    ...
    def __log_indState(self, msg: bytes) -> None:
        ...
        try:
            ...
            self.logger.info(f"IndState: {error:1d} {state:1d} {position:3d} {carriage:5s}({beltShift:11s},{direction:5s}) Hal:{hallActive:5s} ({hallLeft:5d}, {hallRight:5d})")
        except:
            self.logger.info(f"IndState: {error:1d} {state:1d} {position:3d} {carriage:4s}({direction:5s}) Hall:({hallLeft:5d}, {hallRight:5d})")
